  ...
  getters: {
    getQueueCard: (state) => {
      ...
      if (!state.queueCard) {
        ...
        const filteredCards = state.cards.filter((c) => (!c.due || new Date(c.due).getTime() < new Date().getTime())
    ...
    async getNewQueueCard() {
      ...
      try {
        ...
        if (randomType === 'book') {
          ...
          filteredCards = this.cards.filter((c) => (!c.due || new Date(c.due).getTime() < new Date().getTime())
            && (c.is_active === true || c.is_active === "True"));
          if (filteredCards.length > 0) {
            this.cardsLeftToDo = true;
            randomCard = filteredCards[Math.floor(Math.random() * filteredCards.length)];
